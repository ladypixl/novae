...
const quotes = document.querySelectorAll(".slider_content-quote");
...
gsap.set(quotes, { xPercent: (i) => i * 100, opacity: 0, zIndex: 0 });
...
gsap.set(quotes[0], { opacity: 1, filter: window.BLURS.none, zIndex: 900 });
...
function nextSlide() {
  ...
  // Slide sortant
  gsap.to([slides[current], quotes[current], titles[current], testimonials[current], legends[current], numbers[current]], {
    xPercent: -100,
    opacity: 0,
